...
function init(listJSON) {
	...
	const folders = JSON.parse(query.folders)
	const basePath = folders.project
	const indexList = document.getElementById('index-list')
	console.error(indexJSON)
	console.error(folders)
	console.error(basePath)
	for (var key in indexJSON) {
		const srcpath = indexJSON[key]
		console.error(srcpath)
		const [profile, size, index] = key.split('/')
		const li = create('li', indexList)
		const ifrm = create('iframe', indexList)
		ifrm.setAttribute('src', '.' + srcpath)
